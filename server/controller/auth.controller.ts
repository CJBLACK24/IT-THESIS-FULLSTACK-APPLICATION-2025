 // server/app/controller/auth.controller
...
export const registerUser = async (
  ...
  try {
    ...
    // Respond with token (and let client know OTP was sent)
    res.json({
      success: true,
      token,
...
export const resetPassword = async (
  ...
  try {
    ...
    if (!user) {
      res.status(400).json({ success: false, msg: "PIN is invalid or expired" });
      return;
    }
